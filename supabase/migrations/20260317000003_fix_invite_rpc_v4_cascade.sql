-- Force drop and recreate with CASCADE
DROP FUNCTION IF EXISTS public.register_invite_slot(UUID, UUID) CASCADE;
...
DECLARE
  v_invite RECORD;
  v_current_count INT;
  v_expires_at TIMESTAMPTZ DEFAULT NULL;
BEGIN
  ...
  IF v_invite.class_id IS NOT NULL THEN
    IF v_invite.access_duration_days IS NOT NULL AND v_invite.access_duration_days > 0 THEN
      v_expires_at = NOW() + (v_invite.access_duration_days::TEXT || ' days')::INTERVAL;
    END IF;

    INSERT INTO public.student_classes (user_id, class_id, source, enrollment_date, subscription_expires_at)
    VALUES (p_user_id, v_invite.class_id, 'invite'::TEXT, CURRENT_DATE, v_expires_at)
    ON CONFLICT (user_id, class_id) DO UPDATE SET
      subscription_expires_at = COALESCE(v_expires_at, student_classes.subscription_expires_at),
      source = 'invite';
